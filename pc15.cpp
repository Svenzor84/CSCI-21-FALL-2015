 ...
 * Created by Steve Ross-Byers on Oct 4 2015
 */
#include <cassert>
#include <iostream>
#include <string>
...
ShoppingList::~ShoppingList () {
    ...
    //ensure that the dynamic array is not already NULL before clearing memory
    if (items != NULL){
        delete [] items;
        items = NULL;
    }
...
bool ShoppingList::addItem (string theItem) {
    ...
    //if the list is not full add the item, increment the count, and return true
...
string ShoppingList::getItem (unsigned int index) const {
    ...
    //if the index is valid return the requested value
    if (index < itemCount) {
        return items[index];
...
string ShoppingList::removeItem (unsigned int index) {
    ...
    //if the index is valid proceed to remove the item and decrement the count
    if (index < itemCount) {
        
        //save the deleted value
        string toBeDeleted = items[index];
        items[index] = "";
        itemCount--;
        
        //pack the array after removal so that all items are contiguous
...
void ShoppingList::reset (unsigned int newMaxItems) {
    ...
    //if the requested size is valid update max items and reset the count
    if (newMaxItems > 0) {
        maxItems = newMaxItems;
        itemCount = 0;
        
        //ensure that items is not NULL before clearing memory
        if (items != NULL){
            delete [] items;
        }
    ...
    //if the requested size is invalid throw an exception
    } else {
