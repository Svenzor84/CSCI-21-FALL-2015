 ...
 * Created by Steve Ross-Byers on Oct 3 2015
 */
#include <cassert>
#include <cstdlib>
#include <iostream>
...
string* makeDynoString (string contents){
	string* dynoString;
	dynoString = new string(contents);
	return dynoString;
}
...
unsigned int countChars (string* theString, unsigned int& alpha, unsigned int& num){
	...
	//decide whether or not to throw the exception
	if (theString == NULL){
		throw ArrayException("NULL STRING REFERENCE");
		
	//if not proceed to check theString for alpha and numeric characters
	} else {
		alpha = 0;
		num = 0;
		for(int i = 0; i < (*theString).length(); i++){
			
			//increase alpha for each alpha character found
			if (isalpha((*theString).at(i))){
				alpha++;
			}
			
			//increase num for each numeric character found
...
bool findWord (string* theString, string theWord){
	...
	//if not, proceed to check for a match to the string
	} else {
		
		//wordFound is false by default, change to true if the string is found
		bool wordFound = false;
		if ((*theString).find(theWord) != -1){
			wordFound = true;
		}
...
bool replaceWord (string* theString, string oldWord, string newWord){
	...
	//if not, look for the word to be replaced
	} else {
		
		//initialize wordFound to false by default
		bool wordFound = false;
		
		//if the old word is found, replace it with the new word
		if ((*theString).find(oldWord) != -1){
			(*theString).replace((*theString).find(oldWord), oldWord.length(), newWord);
			
			//word found gets switched to true
			wordFound = true;
		}
